...
class MeasurementThread(QThread):
    ...
    def run(self):
        ...
        #    value_task = progress.add_task(f"[purple]Value 1/{len(self.meas.vals)}", total=self.meas.nparams)
        #    cycle_task = progress.add_task(f"[blue]Cycle 1/{self.n_cycles}", total=self.n_cycles)
#
        #    for cycle in range(1,self.n_cycles+1):
        #        for val in self.meas.vals:
        #            result = self.meas.measure_val(self.logger, self.inst, val)
        #            self.filesave.save_result(self.logger,result)
        #            self.update_data.emit(result)
        #            progress.update(value_task, advance=1)
        #        progress.update(value_task, advance=-len(self.meas.vals), description=f"[purple]Value 1/{len(self.meas.vals)}")
