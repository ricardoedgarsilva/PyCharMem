...
class Measurement:
    def __init__(self,logger,config,inst):
        ...
        self.cycle_type = self.params.get(self.name).get('cycle')
        match len(self.cycle_type):
            case 1: 
                self.plot_grid = (2,2)
                self.plot_titles = [[f'Cycle I-V {self.cycle_type}','Resistance vs Time'],
    ...
    def measure_val(self,logger,inst,val):
